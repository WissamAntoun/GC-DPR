...
def get_bert_tensorizer(args, tokenizer=None):
    ...
    return BertTensorizer(
        tokenizer, args.question_sequence_length, args.ctx_sequence_length
    )
...
class BertTensorizer(Tensorizer):
    ...
    def text_to_tensor(
        ...
        text: str,
        title: str = None,
        question_or_ctx: str = "ctx",
        add_special_tokens: bool = True,
    ):
        text = text.strip()

        if question_or_ctx == "ctx":
            ...
            if title:
                token_ids = self.tokenizer.encode(
                    ...
                    max_length=self.ctx_max_length,
            ...
            seq_len = self.question_max_length
            if self.pad_to_max and len(token_ids) < seq_len:
                token_ids = token_ids + [self.tokenizer.pad_token_id] * (
                    seq_len - len(token_ids)
                )
